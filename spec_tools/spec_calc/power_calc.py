...
def fA(center_x, center_y,time, frequency, field, trap_radius = 0.578e-2):
    ...
    fA = math.cos(phi)*math.sin(phi-omega*time)*sp.j1(kc*rho)/(kc*rho) - math.sin(phi)*math.cos(phi-omega*time) * sp.jvp(1,kc*rho)
...
def fB(center_x, center_y,time, frequency, field, trap_radius = 0.578e-2):
    ...
    fB = -math.sin(phi)*math.sin(phi-omega*time)*sp.j1(kc*rho)/(kc*rho) - math.cos(phi)*math.cos(phi-omega*time) * sp.jvp(1,kc*rho)
...
def fA_fB_integral_sum(center_x, center_y, frequency, field, trap_radius = 0.578e-2):
    ...
    fA_cos_integrand_values = [fA(center_x, center_y,time, frequency, field, trap_radius)*math.cos(omega*time)*omega / (2*math.pi) for time in time_values]
    fA_sin_integrand_values =[fA(center_x, center_y,time, frequency, field, trap_radius)*math.sin(omega*time)*omega / (2*math.pi) for time in time_values]
    
    fB_cos_integrand_values =[fB(center_x, center_y,time, frequency, field, trap_radius)*math.cos(omega*time)*omega / (2*math.pi) for time in time_values]
    fB_sin_integrand_values = [fB(center_x, center_y,time, frequency, field, trap_radius)*math.sin(omega*time)*omega / (2*math.pi) for time in time_values]
    ...
    fA_fB_integral_complete = abs(fA_cos_integral)**2 + abs(fA_sin_integral)**2 + abs(fB_cos_integral)**2 + abs(fB_sin_integral)**2
    
    return fA_fB_integral_complete
    
def power_calc(center_x, center_y,frequency,field,trap_radius = 0.578e-2):
    """
    Calculates the average cyclotron radiation power in Watts in the TE11 mode of an electron undergoing cyclotron motion in the cylindrical waveguide around the point (center_x,center_y) with frequency in Hz and field in Tesla.
    """
    ...
    power_constant_denominator = math.pi * beta * (pow(p11prime,2)-1) * pow(sp.j1(p11prime),2)

    power = power_constant_numerator / power_constant_denominator * fA_fB_integral_sum(center_x, center_y, frequency, field, trap_radius)
